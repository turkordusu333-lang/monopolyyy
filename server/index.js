...
const { registerUser, loginUser, getUser, updateUserStats, updateProfile, getLeaderboard, buyCustomization, selectCustomization } = require('./db');
...
io.on('connection', (socket) => {
  ...
  socket.on('getLeaderboard', async (data, cb) => {
    ...
    const lb = await getLeaderboard(period);
    if (callback) callback(lb);
  });

  socket.on('buyCustomization', async ({ username, itemType, itemId, cost }, cb) => {
    const res = await buyCustomization(username, itemType, itemId, cost);
    if (cb) cb(res);
  });

  socket.on('selectCustomization', async ({ username, itemType, itemId }, cb) => {
    const res = await selectCustomization(username, itemType, itemId);
    if (res && res.ok) {
      // Aktif oyun varsa odadaki oyuncuyu da anlık güncelle
      const roomCode = socket.data.roomCode;
      const playerId = socket.data.playerId;
      if (roomCode && playerId && rooms[roomCode]) {
        const gamePlayer = rooms[roomCode].game.players.find(p => p.id === playerId);
        if (gamePlayer) {
          if (itemType === 'border') {
  ...
  socket.on('createRoom', ({ name, settings, dbUsername }, cb) => {
    ...
    // Veritabanı kullanıcısını eşleştir ve özelleştirmelerini yükle
    const gamePlayer = game.players.find(p => p.id === playerId);
    if (gamePlayer && dbUsername) {
      gamePlayer.dbUsername = dbUsername;
      getUser(dbUsername).then(dbRes => {
  ...
  socket.on('addBot', ({ roomCode }, cb) => {
    ...
    const botAvatar = 'bottts'; // Use Dicebear's standard robot style
