...
const CardEffectAnimation: React.FC<{ type: CardEffectType }> = ({ type }) => {
  const [particles, setParticles] = React.useState<{ id: number; x: number; y: number; size: number; color: string; delay: number }[]>([]);
  const [showLightBeam, setShowLightBeam] = React.useState(false);
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => setShowLightBeam(false), 2000);

    // Create particles based on effect type
    let particleCount = 15;
    let colors = ['#F59E0B', '#EF4444', '#10B981', '#3B82F6', '#8B5CF6']; // Rainbow default
    
    if (type === 'steal') {
      colors = ['#C084FC', '#E879F9', '#818CF8']; // Sly/Purple/Pink
      particleCount = 20;
    } else if (type === 'sly-shadow') {
      colors = ['#312E81', '#4C1D95', '#581C87', '#1E1B4B']; // Deep indigo/purple smoke
      particleCount = 25;
    } else if (type === 'debt-seal') {
      colors = ['#DC2626', '#EF4444', '#F87171', '#F59E0B']; // Warning red/orange sparks
      particleCount = 25;
    } else if (type === 'birthday-confetti') {
      colors = ['#EC4899', '#F43F5E', '#10B981', '#3B82F6', '#F59E0B', '#8B5CF6']; // Multicolour party confetti
      particleCount = 30;
    } else if (type === 'gold' || type === 'rent' || type === 'bday') {
      colors = ['#FCD34D', '#F59E0B', '#FFFBEB']; // Gold/Amber
      particleCount = 25;
    } else if (type === 'house') {
      colors = ['#34D399', '#059669', '#A7F3D0']; // Emerald/Green
      particleCount = 18;
    }
  ...
  return (
    <div className="absolute inset-0 z-40 pointer-events-none overflow-hidden rounded-xl">
      {/* 1. Sweeping Light Beam Overlay */}
      {showLightBeam && (
        <motion.div
          initial={{ top: '-100%', opacity: 0 }}
          animate={{ top: ['-100%', '100%'], opacity: [0, 1, 1, 0] }}
          transition={{ duration: 1.2, ease: 'easeInOut' }}
          className="absolute inset-x-0 h-10 bg-gradient-to-b from-transparent via-white/50 to-transparent mix-blend-overlay rotate-[15deg] scale-150"
        />
      )}

      {/* 2. Shimmering Radial Backlight */}
      <motion.div
        initial={{ scale: 0.8, opacity: 0 }}
        animate={{ scale: [0.8, 1.2, 0.9], opacity: [0, 0.6, 0] }}
        transition={{ duration: 1.5, ease: 'easeOut' }}
        className="absolute inset-0 bg-radial from-current via-transparent to-transparent opacity-0"
        style={{
          color: type === 'steal' || type === 'sly-shadow' ? '#A855F7' : type === 'house' ? '#10B981' : type === 'debt-seal' ? '#DC2626' : '#F59E0B',
        }}
      />

      {/* 3. Sly Shadow creeping shadow layer */}
      {type === 'sly-shadow' && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: [0, 0.7, 0.7, 0] }}
          transition={{ duration: 2.2 }}
          className="absolute inset-0 bg-black/60 mix-blend-multiply shadow-[inset_0_0_20px_rgba(0,0,0,0.9)]"
        />
      )}

      {/* 4. Debt Seal stamping overlay */}
      {type === 'debt-seal' && (
        <motion.div
          initial={{ scale: 3, opacity: 0, rotate: -45 }}
          animate={{ scale: [3, 0.9, 0.9, 1.1, 1.1, 0], opacity: [0, 1, 1, 1, 1, 0], rotate: [-45, -12, -12, -12, -12, -30] }}
          transition={{ duration: 2.5, times: [0, 0.15, 0.7, 0.75, 0.9, 1], ease: 'easeOut' }}
          className="absolute inset-0 flex items-center justify-center z-50 pointer-events-none"
        >
          <div className="border-2 border-red-600 bg-red-950/90 text-red-500 font-black text-[9px] px-2 py-0.5 rounded uppercase tracking-wider shadow-[0_0_10px_rgba(220,38,38,0.7)] transform rotate-[-12deg] select-none flex items-center gap-0.5">
            <span>🚫</span> HACİZ <span>🚫</span>
          </div>
        </motion.div>
      )}

      {/* 5. Flying Floating Particles */}
      {particles.map((p) => (
        <motion.div
          key={p.id}
          initial={{ x: `${p.x}%`, y: '90%', opacity: 0, scale: 0.5 }}
          animate={{ 
            y: ['90%', `${p.y - 40}%`], 
      ...
      {/* 6. Falling Coins for gold/rent */}
      {(type === 'gold' || type === 'rent' || type === 'bday') && (
        Array.from({ length: 8 }).map((_, idx) => (
          <motion.div
            key={`coin-${idx}`}
            initial={{ y: -20, x: `${Math.random() * 80 + 10}%`, opacity: 0, rotate: 0 }}
            animate={{ 
              y: ['0%', '120%'], 
              opacity: [0, 1, 1, 0],
      ...
      {/* 7. Birthday Confetti/Balloons */}
      {type === 'birthday-confetti' && (
        Array.from({ length: 6 }).map((_, idx) => (
          <motion.div
            key={`party-${idx}`}
            initial={{ y: 120, x: `${15 + idx * 15}%`, opacity: 0, scale: 0.5 }}
            animate={{ 
              y: ['120%', '-20%'], 
              opacity: [0, 1, 1, 0],
              scale: [0.5, 1.2, 0.8],
              x: [`${15 + idx * 15}%`, `${15 + idx * 15 + (Math.random() * 20 - 10)}%`]
            }}
            transition={{ 
              duration: 2.2, 
              delay: idx * 0.25,
