...
export function VictoryOverlay({ winnerName, myPlayerId, isHost, players, onReturnToLobby, onNewGame, onExit, history = [], lang = 'tr' }) {
  ...
  const getPlayerBadges = (p) => {
    ...
    if (p.stats?.rentCollected > 0 && p.stats?.rentCollected === maxRent) {
      badges.push({ text: lang === 'en' ? '💰 RENT BARON' : '💰 KİRA BARONU', bg: 'rgba(46, 204, 113, 0.15)', border: 'rgba(46, 204, 113, 0.3)', color: '#2ECC71' });
    }
    if (p.stats?.stealsPerformed > 0 && p.stats?.stealsPerformed === maxSteals) {
      badges.push({ text: lang === 'en' ? '🥷 SLY THIEF' : '🥷 SİNSİ HIRSIZ', bg: 'rgba(155, 89, 182, 0.15)', border: 'rgba(155, 89, 182, 0.3)', color: '#9B59B6' });
    }
    if (p.stats?.actionsPlayed > 0 && p.stats?.actionsPlayed === maxActions) {
      badges.push({ text: lang === 'en' ? '⚡ ACTION FIEND' : '⚡ AKSİYON CANAVARI', bg: 'rgba(231, 76, 60, 0.15)', border: 'rgba(231, 76, 60, 0.3)', color: '#E74C3C' });
    }
    if (p.stats?.cardsDrawn > 0 && p.stats?.cardsDrawn === maxCards) {
      badges.push({ text: lang === 'en' ? '🃏 CARD ADDICT' : '🃏 KART KOLİK', bg: 'rgba(52, 152, 219, 0.15)', border: 'rgba(52, 152, 219, 0.3)', color: '#3498DB' });
    }
    if (badges.length === 0) {
      badges.push({ text: lang === 'en' ? '👏 RESPECTED PLAYER' : '👏 SAYGIDEĞER OYUNCU', bg: 'rgba(255, 255, 255, 0.08)', border: 'rgba(255, 255, 255, 0.15)', color: '#aaa' });
    }
    return badges;
  };

  const getColorNameLocal = (color, lg) => {
    if (lg === 'en') {
      const enNames = {
        brown: 'Brown',
      ...
      <div style={{
          ...
          {isMeWinner ? (
            <>
              <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255, 215, 0, 0.1)', border: '1px solid rgba(255, 215, 0, 0.2)', padding: '6px 16px', borderRadius: '20px', gap: '8px', marginBottom: '10px' }}>
                ...
                <span style={{ fontSize: '12px', fontWeight: 900, color: '#FFD700', letterSpacing: '2px', textTransform: 'uppercase' }}>{lang === 'en' ? 'Game Over' : 'Oyun Sonuçlandı'}</span>
              </div>
              <h1 style={{ margin: 0, fontSize: '36px', fontWeight: 950, background: 'linear-gradient(135deg, #FFF 30%, #FFD700 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', textShadow: '0 2px 20px rgba(255,215,0,0.2)' }}>
                {lang === 'en' ? `VICTORY FOR ${winnerName.toUpperCase()}!` : `ZAFER ${winnerName.toUpperCase()}'İN!`}
              </h1>
            </>
          ) : (
            <>
              <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.2)', padding: '6px 16px', borderRadius: '20px', gap: '8px', marginBottom: '10px' }}>
                <span style={{ fontSize: '18px' }}>💀</span>
                <span style={{ fontSize: '12px', fontWeight: 900, color: '#f87171', letterSpacing: '2px', textTransform: 'uppercase' }}>{lang === 'en' ? 'DEFEAT' : 'BOZGUN'}</span>
              </div>
        ...
        <div style={{
            ...
            {winnerCompleteSets.length > 0 && (
              <div style={{ width: '100%', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '20px' }}>
                <h3 style={{ margin: '0 0 16px 0', color: '#fff', fontSize: '14px', fontWeight: 800, letterSpacing: '1.5px', textTransform: 'uppercase' }}>
                  {lang === 'en' ? '🏆 Completed Sets' : '🏆 Tamamlanmış Setler'}
                </h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '14px', fontWeight: 900, color: '#aaa', letterSpacing: '1px' }}>{lang === 'en' ? 'LEADERBOARD' : 'LİDERLİK TABLOSU'}</span>
              <span style={{ fontSize: '11px', color: '#666', fontWeight: 700 }}>{lang === 'en' ? 'Scroll for details' : 'Detaylar için kaydırın'}</span>
            </div>
                  ...
                  <div key={p.id} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px', marginBottom: '12px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontSize: '10px', background: 'rgba(255,255,255,0.08)', color: '#FFD700', border: '1px solid rgba(255,255,255,0.1)', padding: '3px 8px', borderRadius: '12px', fontWeight: 900 }}>
                          🏆 {lang === 'en' ? `${playerCompleteSets} FULL SETS` : `${playerCompleteSets} TAM SET`}
                        </span>
                      </div>
                    </div>
                    ...
                    <div style={{
                      ...
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '8.5px', color: '#718096', marginBottom: '2px' }}>{lang === 'en' ? 'Rent' : 'Kira'}</div>
                        <div style={{ fontSize: '11px', fontWeight: 900, color: '#2ECC71' }}>{p.stats?.rentCollected || 0}M</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '8.5px', color: '#718096', marginBottom: '2px' }}>{lang === 'en' ? 'Vault' : 'Kasa'}</div>
                        <div style={{ fontSize: '11px', fontWeight: 900, color: '#E2E8F0' }}>{p.stats?.totalMoneyBanked || 0}M</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '8.5px', color: '#718096', marginBottom: '2px' }}>{lang === 'en' ? 'Drawn' : 'Çekilen'}</div>
                        <div style={{ fontSize: '11px', fontWeight: 900, color: '#3498DB' }}>{p.stats?.cardsDrawn || 0}</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '8.5px', color: '#718096', marginBottom: '2px' }}>{lang === 'en' ? 'Played' : 'Oynanan'}</div>
                        <div style={{ fontSize: '11px', fontWeight: 900, color: '#9B59B6' }}>{p.stats?.cardsPlayed || 0}</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '8.5px', color: '#718096', marginBottom: '2px' }}>{lang === 'en' ? 'Act.' : 'Aks.'}</div>
                        <div style={{ fontSize: '11px', fontWeight: 900, color: '#E74C3C' }}>{p.stats?.actionsPlayed || 0}</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '8.5px', color: '#718096', marginBottom: '2px' }}>{lang === 'en' ? 'No' : 'Reddet'}</div>
                        <div style={{ fontSize: '11px', fontWeight: 900, color: '#16A085' }}>{p.stats?.justSayNoCount || 0}</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '8.5px', color: '#718096', marginBottom: '2px' }}>{lang === 'en' ? 'Steal' : 'Çalma'}</div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                <h3 style={{ margin: 0, color: '#FFD700', fontSize: '15px', fontWeight: 800, letterSpacing: '1px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {lang === 'en' ? '📈 NET WORTH HISTORY (MATCH ANALYTICS)' : '📈 SERVET GELİŞİM GRAFİĞİ (MATCH ANALYTICS)'}
                </h3>
                <span style={{ fontSize: '10px', color: '#666', fontWeight: 700 }}>{lang === 'en' ? 'Total net worth at the end of each turn (Bank + Property)' : 'Her tur sonundaki toplam servet (Banka + Arazi)'}</span>
              </div>
                      ...
                      <g key={player.id}>
                        ...
                        {playerPts.map((pt, ptIdx) => (
                          <g key={ptIdx} className="chart-dot-group">
                            ...
                            />
                            <title>{`${player.name} - ${lang === 'en' ? 'Turn' : 'Tur'} ${pt.turn}: ${pt.val}M`}</title>
                          </g>
                        ))}
                      </g>
                    );
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center', width: '100%' }}>
            ...
            >
              {lang === 'en' ? '🚪 Return to Lobby' : '🚪 Lobiye Dön'}
            </button>
            ...
            {isHost && (
              ...
              >
                {lang === 'en' ? '🔄 Start New Game' : '🔄 Yeni Oyun Başlat'}
              </button>
            )}
          ...
          <div style={{ width: '100%', maxWidth: '400px', textAlign: 'center' }}>
            <div style={{ fontSize: '12px', color: '#718096', fontWeight: 800, letterSpacing: '1px', marginBottom: '8px' }}>
              {lang === 'en' ? `⏳ RETURNING TO LOBBY: ${timeLeft} SECONDS` : `⏳ LOBİYE GERİ DÖNÜLÜYOR: ${timeLeft} SANİYE`}
            </div>
            ...
            {!isHost && (
              <div style={{ color: '#4a4a5a', fontSize: '10px', marginTop: '8px', fontWeight: 600 }}>
                {lang === 'en' ? 'The host can start a new game, or you can wait to be redirected to the lobby.' : 'Host yeni oyunu başlatabilir veya lobiye yönlendirilmesini bekleyebilirsiniz.'}
              </div>
            )}
          </div>
        </div>
