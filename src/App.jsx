...
export default function App() {
  ...
  const handleBuyCustomization = (itemType, itemId, cost) => {
  ...
  const handleDragEnd = (event, info, card) => {
    ...
    if (target && draggedRef.current) {
      ...
      } else if (info.offset.y > 100 || info.offset.x > 120) {
        if (card.type === 'property') {
          setError('Tapu Senedi kartları bankaya konamaz!');
          sfxError();
        } else {
  ...
  const renderModal = () => {
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <img
                    ...
                    style={{
                      ...
                      borderRadius: '50%',
                      background: 'rgba(255,255,255,0.05)',
                      border: item.selectedBorder && item.selectedBorder !== 'default' ?
                        {
                          gold: '2px solid #FFD700',
                          neon: '2px solid #ff00ff',
                          cyber: '2px solid #39ff14',
                          flame: '2px solid #ff4500'
                        }[item.selectedBorder] : 'none',
                      boxShadow: item.selectedBorder && item.selectedBorder !== 'default' ?
                        {
                          gold: '0 0 6px #FFD700',
                          neon: '0 0 6px #ff00ff',
                          cyber: '0 0 6px #39ff14',
                          flame: '0 0 8px #ff4500'
                        }[item.selectedBorder] : 'none',
                    }}
                  />
    ...
    if (modal.type === 'shop') {
      ...
      return (
        <Modal title={lang === 'en' ? "🛍️ CUSTOMIZATION SHOP" : "🛍️ ÖZELLEŞTİRME MAĞAZASI"} onClose={() => setModal(null)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 320, maxWidth: 500, maxHeight: 420, overflowY: 'auto', paddingRight: 6 }}>
            {/* Points balance */}
            <div style={{
              display: 'flex',
              justifyContent: 'space-between',
            ...
            {/* Borders Section */}
            <div>
              <h3 style={{ fontSize: 12, color: '#94a3b8', borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: 4, marginBottom: 10, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                {lang === 'en' ? 'Profile Borders' : 'Profil Çerçeveleri'}
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {/* Default Border */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.02)', padding: 10, borderRadius: 10, border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: '1px solid #aaa', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>👤</div>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 'bold' }}>{lang === 'en' ? 'Classic Border' : 'Klasik Çerçeve'}</div>
                      <div style={{ fontSize: 10, color: '#64748b' }}>{lang === 'en' ? 'Standard profile look' : 'Standart profil görünümü'}</div>
                    </div>
                  </div>
                  {currentBorder === 'default' ? (
                    <span style={{ fontSize: 10, fontWeight: 900, color: '#39ff14', background: 'rgba(57,255,20,0.1)', padding: '4px 8px', borderRadius: 6 }}>{lang === 'en' ? 'ACTIVE' : 'AKTİF'}</span>
                  ) : (
                    <button
                      onClick={() => handleSelectCustomization('border', 'default')}
                      style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', fontSize: 10, fontWeight: 'bold', padding: '5px 10px', borderRadius: 8, cursor: 'pointer' }}
                    >
                      {lang === 'en' ? 'EQUIP' : 'KUŞAN'}
                    </button>
                  )}
                ...
                {[
                  ...
                  return (
                    <div key={item.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.02)', padding: 10, borderRadius: 10, border: `1px solid ${active ? item.color : 'rgba(255,255,255,0.05)'}`, boxShadow: active ? `0 0 8px ${item.color}` : 'none' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(255,255,255,0.05)', border: `2px solid ${item.color}`, boxShadow: `0 0 5px ${item.color}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>👤</div>
                        <div>
                          <div style={{ fontSize: 12, fontWeight: 'bold', color: item.color }}>{lang === 'en' ? item.name.en : item.name.tr}</div>
                          <div style={{ fontSize: 10, color: '#64748b' }}>{lang === 'en' ? item.desc.en : item.desc.tr}</div>
                        </div>
                      </div>
                      ...
                      {active ? (
                        ...
                        <button
                          onClick={() => handleBuyCustomization('border', item.id, item.cost)}
                          disabled={points < item.cost}
                          style={{
                            background: points >= item.cost ? 'linear-gradient(135deg, #f1c40f, #f39c12)' : 'rgba(255,255,255,0.05)',
                            color: points >= item.cost ? '#000' : '#555',
                            border: 'none',
                            fontSize: 10,
                            fontWeight: 'bold',
                            padding: '6px 12px',
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.02)', padding: 10, borderRadius: 10, border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 32, height: 44, borderRadius: 6, background: 'linear-gradient(135deg, #c0392b, #78281f)', border: '1px solid #f1c40f', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>🎲</div>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 'bold' }}>{lang === 'en' ? 'Classic Red' : 'Klasik Kırmızı'}</div>
                      <div style={{ fontSize: 10, color: '#64748b' }}>{lang === 'en' ? 'Standard Monopoly Deal style' : 'Standart Monopoly Deal stili'}</div>
                    </div>
                  </div>
                  {currentCardBack === 'default' ? (
                    ...
                    <button
                      onClick={() => handleSelectCustomization('cardBack', 'default')}
                      style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', fontSize: 10, fontWeight: 'bold', padding: '5px 10px', borderRadius: 8, cursor: 'pointer' }}
                    >
                      {lang === 'en' ? 'EQUIP' : 'KUŞAN'}
                    </button>
                ...
                {[
                  ...
                  return (
                    <div key={item.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.02)', padding: 10, borderRadius: 10, border: `1px solid ${active ? item.border : 'rgba(255,255,255,0.05)'}`, boxShadow: active ? `0 0 8px ${item.border}` : 'none' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 32, height: 44, borderRadius: 6, background: item.bg, border: `1.5px solid ${item.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>{item.symbol}</div>
                        <div>
                          <div style={{ fontSize: 12, fontWeight: 'bold', color: item.border }}>{lang === 'en' ? item.name.en : item.name.tr}</div>
                          <div style={{ fontSize: 10, color: '#64748b' }}>{lang === 'en' ? item.desc.en : item.desc.tr}</div>
                        </div>
                      </div>
                      ...
                      {active ? (
                        ...
                        <button
                          onClick={() => handleBuyCustomization('cardBack', item.id, item.cost)}
                          disabled={points < item.cost}
                          style={{
                            background: points >= item.cost ? 'linear-gradient(135deg, #f1c40f, #f39c12)' : 'rgba(255,255,255,0.05)',
                            color: points >= item.cost ? '#000' : '#555',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', padding: '4px 10px', borderRadius: 20 }}>
                <img
                  ...
                  style={{
                    ...
                    border: dbUser.selectedBorder && dbUser.selectedBorder !== 'default' ?
                      {
                        gold: '1.5px solid #FFD700',
                        neon: '1.5px solid #ff00ff',
                        cyber: '1.5px solid #39ff14',
                        flame: '1.5px solid #ff4500'
                      }[dbUser.selectedBorder] : 'none',
                    boxShadow: dbUser.selectedBorder && dbUser.selectedBorder !== 'default' ?
                      {
                        gold: '0 0 4px #FFD700',
                        neon: '0 0 4px #ff00ff',
                        cyber: '0 0 4px #39ff14',
                        flame: '0 0 5px #ff4500'
                      }[dbUser.selectedBorder] : 'none',
                  }}
                />
                      ...
                      >
                        ...
                        <div style={{ position: 'relative', width: 38, height: 52 }}>
                          ...
                          {(!me.bank || me.bank.length === 0) && (
                            <div style={{ width: 32, height: 48, borderRadius: 4, border: '1.2px dashed rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, color: '#444' }}>{TRANSLATIONS[lang]?.empty || 'BOŞ'}</div>
                          )}
                        </div>
                      </div>
                  ...
                  {handToRender.map((card, idx) => {
                    ...
                    return (
                      <motion.div
                        ...
                        drag={isMyTurn && !isBlocked && !discardMode ? true : false}
                        dragElastic={0.2}
